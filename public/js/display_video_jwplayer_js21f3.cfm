...
var playerConfigmediaspaceShowModal = {
	"playlist": [{
		"sources": [
			...
			{ "type": "hls", "label": "Auto-HLS", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683709411_aeb5e31d913bf9262e2fe679c81d09186512beea/2020/02/carrie-pages/6374_marketing/video/h264-,504p,720p,360p,216p.m3u8" },
			{ "type": "mpd", "label": "Auto-MPD", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683709411_aeb5e31d913bf9262e2fe679c81d09186512beea/2020/02/carrie-pages/6374_marketing/video/h264-,504p,720p,360p,216p.mpd" }, {}
			
		],
		"image": "//images.pilatesanytime.com/2020/05/05/large_carrie_200212_PA6374-13258.jpg",
		
		"tracks": [
			
			{ "kind": "thumbnails", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683709411_ecbc174db111cdb6146354c97f0fb8f41d385067/2020/02/carrie-pages/6374_marketing/thumbs/160x90.vtt" },
...
if (typeof initJWPlayerMux !== 'undefined') {
	initJWPlayerMux(playermediaspaceShowModal, {
		...
		data: {
			...
			video_id: '8125', // ex: 'abcd123'
			video_title: '8125: ', // ex: 'My Great Video'
...
playermediaspaceShowModal.on('pause', function(data) { 
	...
	videoIsPlaying = 0;
	if (document.ga) ga('send', 'event', 'video', 'pause', 'video_id', 8125);
});
...
playermediaspaceShowModal.on('play', function(data) {
	...
	if (document.ga) ga('send', 'event', 'video', 'play', 'video_id', 8125);
...
playermediaspaceShowModal.on('complete', function() {
	...
	if (document.ga) ga('send', 'event', 'video', 'complete', 'video_id', 8125);
...
playermediaspaceShowModal.on('error', function(error) {
	...
	if (document.ga) ga('send', 'event', 'video', 'error', 'video_id', 8125);
...
function markWatched(my_action) {		
	...
	if (my_action == 'mediaPlay' || my_action == 'mediaStop' || my_action == 'mediaComplete' || my_action == 'mediaProgress') {
		var myResult = instance.saveClassView(8125, -1, '-1', my_action, -1, my_position);
	}
