...
var playerConfigmediaspaceClassModal = {
	"playlist": [{
		"sources": [
			...
			{ "type": "hls", "label": "Auto-HLS", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683709587_d379499b26ef0e700673a6a0136cd2e83f65b210/2022/06/kira-lamb/7762_marketing_01/video/h264-,504p,720p,360p,216p.m3u8" },
			{ "type": "mpd", "label": "Auto-MPD", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683709587_d379499b26ef0e700673a6a0136cd2e83f65b210/2022/06/kira-lamb/7762_marketing_01/video/h264-,504p,720p,360p,216p.mpd" }, {}
			
		],
		"image": "//images.pilatesanytime.com/2022/12/08/large_full_kiralamb7762TIMG731365858-20344.jpg",
		
		"tracks": [
			
			{ "kind": "thumbnails", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683709587_714ffa84697b3fb4ba7892252cbfb97d988cb135/2022/06/kira-lamb/7762_marketing_01/thumbs/160x90.vtt" },
...
if (typeof initJWPlayerMux !== 'undefined') {
	initJWPlayerMux(playermediaspaceClassModal, {
		...
		data: {
			...
			video_id: '9389', // ex: 'abcd123'
			video_title: '9389: ', // ex: 'My Great Video'
...
playermediaspaceClassModal.on('pause', function(data) { 
	...
	videoIsPlaying = 0;
	if (document.ga) ga('send', 'event', 'video', 'pause', 'video_id', 9389);
});
...
playermediaspaceClassModal.on('play', function(data) {
	...
	if (document.ga) ga('send', 'event', 'video', 'play', 'video_id', 9389);
...
playermediaspaceClassModal.on('complete', function() {
	...
	if (document.ga) ga('send', 'event', 'video', 'complete', 'video_id', 9389);
...
playermediaspaceClassModal.on('error', function(error) {
	...
	if (document.ga) ga('send', 'event', 'video', 'error', 'video_id', 9389);
...
function markWatched(my_action) {		
	...
	if (my_action == 'mediaPlay' || my_action == 'mediaStop' || my_action == 'mediaComplete' || my_action == 'mediaProgress') {
		var myResult = instance.saveClassView(9389, -1, '-1', my_action, -1, my_position);
	}
