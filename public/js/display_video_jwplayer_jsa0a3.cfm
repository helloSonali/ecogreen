...
var playerConfigmediaspaceShowModal = {
	"playlist": [{
		"sources": [
			...
			{ "type": "hls", "label": "Auto-HLS", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683709400_d66f1e493a8eb7db868ceba066bfadd6f8eca205/2019/08/courtney-miller/6180_marketing/video/h264-,504p,720p,360p,216p.m3u8" },
			{ "type": "mpd", "label": "Auto-MPD", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683709400_d66f1e493a8eb7db868ceba066bfadd6f8eca205/2019/08/courtney-miller/6180_marketing/video/h264-,504p,720p,360p,216p.mpd" }, {}
			
		],
		"image": "//images.pilatesanytime.com/2019/11/18/large_welcome1-87768.jpg",
		
		"tracks": [
			
			{ "kind": "thumbnails", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683709400_72a9b695ed17b1f1953efbf1b3d6207e33f77c91/2019/08/courtney-miller/6180_marketing/thumbs/160x90.vtt" },
...
if (typeof initJWPlayerMux !== 'undefined') {
	initJWPlayerMux(playermediaspaceShowModal, {
		...
		data: {
			...
			video_id: '7882', // ex: 'abcd123'
			video_title: '7882: ', // ex: 'My Great Video'
...
playermediaspaceShowModal.on('pause', function(data) { 
	...
	videoIsPlaying = 0;
	if (document.ga) ga('send', 'event', 'video', 'pause', 'video_id', 7882);
});
...
playermediaspaceShowModal.on('play', function(data) {
	...
	if (document.ga) ga('send', 'event', 'video', 'play', 'video_id', 7882);
...
playermediaspaceShowModal.on('complete', function() {
	...
	if (document.ga) ga('send', 'event', 'video', 'complete', 'video_id', 7882);
...
playermediaspaceShowModal.on('error', function(error) {
	...
	if (document.ga) ga('send', 'event', 'video', 'error', 'video_id', 7882);
...
function markWatched(my_action) {		
	...
	if (my_action == 'mediaPlay' || my_action == 'mediaStop' || my_action == 'mediaComplete' || my_action == 'mediaProgress') {
		var myResult = instance.saveClassView(7882, -1, '-1', my_action, -1, my_position);
	}
