...
var playerInitTimemediaspace99 = Date.now();
var playermediaspace99 = jwplayer("mediaspace99");

var playerConfigmediaspace99 = {
	"playlist": [{
		"sources": [
			

			{ "type": "hls", "label": "Auto-HLS", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683709992_4b166d8bb90728dd8bfb3f2d1c55d2299861bc21/2009/12/meredith-rogers/3101/video/h264-,504p,720p,360p,216p.m3u8?subtitles=en" },
			{ "type": "mpd", "label": "Auto-MPD", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683709992_4b166d8bb90728dd8bfb3f2d1c55d2299861bc21/2009/12/meredith-rogers/3101/video/h264-,504p,720p,360p,216p.mpd" }, {}
			
		],
		"image": "//images.pilatesanytime.com/2014/08/08/large_Screen-Shot-2014-08-08-at-2.07-PM-67226.jpg",
		
		"tracks": [
			
			{ "kind": "thumbnails", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683709992_cc0e0c80811627ee54b7da99c2ab419f26826298/2009/12/meredith-rogers/3101/thumbs/160x90.vtt" },
...
playermediaspace99.setup(playerConfigmediaspace99);



if (typeof initJWPlayerMux !== 'undefined') {
	initJWPlayerMux(playermediaspace99, {
		debug: false,
		data: {
			...
			player_init_time: playerInitTimemediaspace99, // ex: 1451606400000

			// Video Metadata (cleared with 'videochange' event)
			video_id: '99', // ex: 'abcd123'
			video_title: '99: ', // ex: 'My Great Video'
...
playermediaspace99.on('time', function(evt) { 
...
playermediaspace99.on('pause', function(data) { 
	if (videoIsPlaying == 1) { markWatched("mediaStop"); }
	videoIsPlaying = 0;
	if (document.ga) ga('send', 'event', 'video', 'pause', 'video_id', 99);
});




playermediaspace99.on('play', function(data) {
	
	
	if (document.ga) ga('send', 'event', 'video', 'play', 'video_id', 99);

	
});


playermediaspace99.on('complete', function() {
	
	if (document.ga) ga('send', 'event', 'video', 'complete', 'video_id', 99);
...
playermediaspace99.on('error', function(error) {

	if (document.ga) ga('send', 'event', 'video', 'error', 'video_id', 99);
	...
	var payload = {
		...
		i: "-1",
		e: error,
		c: jwplayer('mediaspace99').getConfig()
	};
	...
	if (error.message.indexOf("Error loading media: File could not be played") >= 0) {
		...
		var file = jwplayer('mediaspace99').getConfig().playlist[0].file;
...
function doPausemediaspace99() {
	if(playermediaspace99.getState() == "playing") {
		playermediaspace99.pause();
	}
}

function doPlayPausemediaspace99() {
	if(playermediaspace99.getState() == "playing") {
		playermediaspace99.pause();
	}
	else {
		playermediaspace99.play();
	}
}

function doResumemediaspace99(resume_position) {
	playermediaspace99.seek(resume_position);
...
function markWatched(my_action) {		
	...
	if (my_action == 'mediaPlay' || my_action == 'mediaStop' || my_action == 'mediaComplete' || my_action == 'mediaProgress') {
		var myResult = instance.saveClassView(99, -1, '-1', my_action, -1, my_position);
	}
...
function seek(timecode) { playermediaspace99.seek(timecode); }

function playVideo() { playermediaspace99.play(); }
