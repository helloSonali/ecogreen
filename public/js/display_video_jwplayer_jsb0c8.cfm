...
var playerConfigmediaspaceShowModal = {
	"playlist": [{
		"sources": [
			...
			{ "type": "hls", "label": "Auto-HLS", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683708623_8baa24cbf07428d3e74ea0e10dcfab6ccbb7ada0/2022/05/laura-hanlon/7678_marketing/video/h264-,504p,720p,360p,216p.m3u8" },
			{ "type": "mpd", "label": "Auto-MPD", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683708623_8baa24cbf07428d3e74ea0e10dcfab6ccbb7ada0/2022/05/laura-hanlon/7678_marketing/video/h264-,504p,720p,360p,216p.mpd" }, {}
			
		],
		"image": "//images.pilatesanytime.com/2022/07/15/large_laura_220602_PA7678_TIMG_landscape-72835.jpg",
		
		"tracks": [
			
			{ "kind": "thumbnails", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683708623_dcbf1b427b7d395c6f59c66d2b962124976aa22e/2022/05/laura-hanlon/7678_marketing/thumbs/160x90.vtt" },
...
if (typeof initJWPlayerMux !== 'undefined') {
	initJWPlayerMux(playermediaspaceShowModal, {
		...
		data: {
			...
			video_id: '9206', // ex: 'abcd123'
			video_title: '9206: ', // ex: 'My Great Video'
...
playermediaspaceShowModal.on('pause', function(data) { 
	...
	videoIsPlaying = 0;
	if (document.ga) ga('send', 'event', 'video', 'pause', 'video_id', 9206);
});
...
playermediaspaceShowModal.on('play', function(data) {
	...
	if (document.ga) ga('send', 'event', 'video', 'play', 'video_id', 9206);
...
playermediaspaceShowModal.on('complete', function() {
	...
	if (document.ga) ga('send', 'event', 'video', 'complete', 'video_id', 9206);
...
playermediaspaceShowModal.on('error', function(error) {
	...
	if (document.ga) ga('send', 'event', 'video', 'error', 'video_id', 9206);
...
function markWatched(my_action) {		
	...
	if (my_action == 'mediaPlay' || my_action == 'mediaStop' || my_action == 'mediaComplete' || my_action == 'mediaProgress') {
		var myResult = instance.saveClassView(9206, -1, '-1', my_action, -1, my_position);
	}
