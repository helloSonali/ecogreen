...
var playerConfigmediaspaceShowModal = {
	"playlist": [{
		"sources": [
			...
			{ "type": "hls", "label": "Auto-HLS", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683708548_712fefd915b6ca0ef0670267736d2b768eacc610/2022/02/courtney-miller/7552/video/h264-,504p,720p,360p,216p.m3u8?subtitles=en" },
			{ "type": "mpd", "label": "Auto-MPD", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683708548_712fefd915b6ca0ef0670267736d2b768eacc610/2022/02/courtney-miller/7552/video/h264-,504p,720p,360p,216p.mpd" }, {}
			
		],
		"image": "//images.pilatesanytime.com/2022/05/16/large_courtney_220218_PA7552-62785.jpg",
		
		"tracks": [
			
			{ "kind": "thumbnails", "file": "https://pilatesanytime-media.global.ssl.fastly.net/1683708548_3df9a9dff84395c902d16f58914feed6121f82b6/2022/02/courtney-miller/7552/thumbs/160x90.vtt" },
...
if (typeof initJWPlayerMux !== 'undefined') {
	initJWPlayerMux(playermediaspaceShowModal, {
		...
		data: {
			...
			video_id: '9123', // ex: 'abcd123'
			video_title: '9123: ', // ex: 'My Great Video'
...
playermediaspaceShowModal.on('pause', function(data) { 
	...
	videoIsPlaying = 0;
	if (document.ga) ga('send', 'event', 'video', 'pause', 'video_id', 9123);
});
...
playermediaspaceShowModal.on('play', function(data) {
	...
	if (document.ga) ga('send', 'event', 'video', 'play', 'video_id', 9123);
...
playermediaspaceShowModal.on('complete', function() {
	...
	if (document.ga) ga('send', 'event', 'video', 'complete', 'video_id', 9123);
...
playermediaspaceShowModal.on('error', function(error) {
	...
	if (document.ga) ga('send', 'event', 'video', 'error', 'video_id', 9123);
...
function markWatched(my_action) {		
	...
	if (my_action == 'mediaPlay' || my_action == 'mediaStop' || my_action == 'mediaComplete' || my_action == 'mediaProgress') {
		var myResult = instance.saveClassView(9123, -1, '-1', my_action, -1, my_position);
	}
